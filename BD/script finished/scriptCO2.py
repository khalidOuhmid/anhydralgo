...
password = 'fqnsbtc4'
fichiers = [
    'Z:\Documents\saeTruc\co-emissions-per-capita.csv',
]
        ...
        if country_id:
            ...
            if date_id:
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?)
                """, country_id, date_id, value, "Annual CO2 emissions per capita", unit)
                conn.commit()

def process_file(filename, conn):
    df = pd.read_csv(filename, delimiter=',', names=['Entity', 'Code', 'Year', 'Annual CO2 emissions (per capita)'], skiprows=1)
    
    for index, row in df.iterrows():
        try:
            value = float(row['Annual CO2 emissions (per capita)'])
            if value != 0.0:
                process_row(row['Entity'], row['Code'], row['Year'], value, conn)
            else:
                print(f"Valeur vide ou zéro pour la ligne {index}")
        except ValueError:
            print(f"Valeur non numérique pour la ligne {index}: {row['Annual CO2 emissions (per capita)']}")
