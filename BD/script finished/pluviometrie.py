...
password = 'fqnsbtc4'
fichiers = [
    'Z:/Documents/saeTruc/pluvio.csv',
]
...
cont = ['North America', 'South America', 'Asia', 'Australia', 'Africa', 'Europe', 'World', 'Caribbean',
        ...
        'Bahamas', 'Channel', 'Dem', 'Rep', 'Curacao', 'Hong','Kosovo']
...
def process_row(country, date, precipitation, conn):
    region_name, country_name = get_region_and_country(country)
        ...
        if country_id:
            ...
            if date_id:
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?)
                """, country_id, date_id, precipitation, "Annual Precipitation", unit)
                conn.commit()

def process_file(filename, conn):
    df = pd.read_csv(filename, delimiter=',', names=['Country', 'Date', 'Rank', 'Precipitation'], skiprows=1, on_bad_lines='skip')
    ...
    for index, row in df.iterrows():
        try:
            ...
            if year >= 1950:
                ...
                process_row(row['Country'], row['Date'], precipitation, conn)
            else:
            ...
            print(f"Erreur inattendue pour la ligne {index}: {e}")
